...
class GUI:
    ...
    def choose_card(self):
        """
        wait for player to choose(click on) card
        :return: card_pressed - the card the player choose
        """
        while True:
    ...
    def display_message(self, text):
        ...
        text_font_highlight = pygame.font.SysFont("Comic Sans MS", 40)
    ...
    #     self.create_screen()
    #     self.print_cards()
    #     # game = GAME(self.discard_pile, 0, self.removed_cards[self.player_num-1])
    #     game = GAME.GAME(self.discard_pile, 0, self.removed_cards)
    #
    #     print("Are cards valid?", game.have_valid_card())
    #     if not game.have_valid_card():
    #         self.display_message("No Valid Card")
    #         self.restart()
    #         return
    #     # print what player it is
    #     font = pygame.font.Font(None, 36)
    #     number_text = font.render(str(self.player_num + 1), True, BLACK)  # Convert integer to string
    #     self.screen.blit(number_text, (0, 0))
    #     pygame.display.flip()
    #
    #     self.card_pressed = self.choose_card()
    #     game = GAME.GAME(self.discard_pile, self.card_value, self.removed_cards[self.player_num - 1])
    #     print("Is card valid?", game.is_card_valid())
    #     while not game.is_card_valid():
    #         self.display_message("Card Chosen INVALID")
    #         self.create_screen()
    #         self.print_cards()
    #         self.card_pressed = self.choose_card()
    #         game = GAME.GAME(self.discard_pile, self.card_value, self.removed_cards[self.player_num - 1])
    #     self.move_to_middle(self.removed_cards, self.player_num, self.card_pressed)
    #     next_card = self.replace_chosen_card(self.card_pressed, self.removed_cards, self.player_num)
    #     if next_card is not False:
    #         self.draw_new_card(self.card_pressed)
    #     else:
    #         if len(self.removed_cards) == 2:
    #             self.draw_two_cards()
    #         elif len(self.removed_cards) == 1:
    #             self.draw_last_cards()
    #         else:
    #             print("Player Won")
    #             return
    #     print("End Game")
